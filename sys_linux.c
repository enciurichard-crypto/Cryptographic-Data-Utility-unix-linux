...
size_t gather_random_fast(void* pbuffer, size_t bytes) {
	return sample_from_device( pbuffer, bytes, enum_urandom );
}

size_t gather_random_slow(void* pbuffer, size_t bytes) {
	return sample_from_device( pbuffer, bytes, enum_random );
}
...
size_t sample_from_device( void* ubuffer, size_t bytes, int devt )
{
	struct timeval tv;
	struct stat lstat;
	fd_set setfd;
